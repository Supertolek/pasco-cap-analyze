# ZRS : Z (Z-axis I believe) Render Set
# ZTDDRBP : ZTwoDimensionDataRenderBasicProperties
import sys
import os
import re
...
def transpose(array):
    ...
    extended_array = [
        list(sublist) + [0] * (max_length - len(sublist)) for sublist in array
    ]
...
dependent_file_pattern = re.compile(
    r"<DependentStorageElement [^>]*FileName=\"([^\"]+)\""
)
independent_file_pattern = re.compile(
    r"<IndependentStorageElement [^>]*FileName=\"([^\"]+)\""
)
time_step_pattern = re.compile(
    r"<IndependentStorageElement [^>]*IntervalCacheInterval=\"([^\"]+)\""
)
...
def extract_data_sets(xml_segment):
    ...
    print(
        "Could not parse: dependent_files, independent_files, time_steps, group_numbers, data_sizes =",
        dependent_files,
        independent_files,
        time_steps,
        group_numbers,
...
def process_archive(archive: ZipFile, output_directory: str) -> str:
    ...
    for data_source_element in data_source_elements:
        ...
        if not data_set_elements:
            ...
            for data_set_element in data_set_elements:
                ...
                data_sets[group_number][
                    measurment_name + ("-" + channel_id_name if channel_id_name else "")
                ] = (independent_file_name, dependent_file_name, data_size)
    print(data_sets)
    ...
    #     I couldn't figure out how to do it, so it stays like that.
    # -------------------------------------------------------------------------------------------------------
    curve_fit_parameters = {}
    ZRSIndividualRenederer = ""  # Are they dumb?
    for renderer_segment in segment(main_xml_content_str, "<ZRSIndividualRenederer"):
        ...
        parameter_segments = segment(
            renderer_segment, "<ZCFDICurveFitParameterDefinition"
        )
        ...
        if set_numbers and slopes and intercepts:
            curve_fit_parameters[int(set_numbers[0])] = (
                float(intercepts[0]),
                float(slopes[0]),
            )
    ...
    for group_number, group_data in sorted(data_sets.items(), key=lambda item: item[0]):
        ...
        legend_index = 0
        for label, (x_data, y_data, data_size) in sorted(
            group_data.items(), key=lambda item: item[0]
        ):
            ...
            if isinstance(x_data, float):
                ...
                combined_data = grok(x_data, data_size, archive) + grok(
                    y_data, data_size, archive
                )
